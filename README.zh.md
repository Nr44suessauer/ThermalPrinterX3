...
![X3 热敏打印机 – 桌面程序（中文界面，带实时预览）](docs/images/gui-zh.png)

*桌面程序：左侧是操作项，右侧是精确到毫米的实时预览
（此处显示网站 `www.deadlinedriven.dev`，与二维码预设内容相同）。*

**「图片」选项卡** – 用阈值、抖动、反色、旋转、拉伸与裁边处理照片、标识和矢量图；
预览会先显示黑白处理后的结果：

![X3 热敏打印机 – 图片选项卡：阈值、抖动、反色、旋转与拉伸](docs/images/gui-image-zh.png)

*「图片」选项卡与项目自带的示例图片（`assets/sample.jpg`）：阈值 190 %、启用抖动、
毫米级尺寸标注、可用蓝色手柄拖动和缩放——所有设置都会保存并在下次打印时继续使用。*
