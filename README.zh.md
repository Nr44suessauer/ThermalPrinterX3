...
* **以系统语言启动**（中文、English、德语、日语）；安装程序的消息和菜单项也会**本地化**。
  可随时在「设置 → 界面 → 语言」中更改
...
| **无法连接**（`timed out`） | 蓝牙会话已失效——X3 同时只允许一个客户端。驱动程序会自动断开并重试（第一次打印会多花几秒）。手动：`bluetoothctl disconnect <MAC>`，或关开打印机 |
| `Device or resource busy` | 打印机仍在处理上一个任务，稍等片刻，驱动会自动重试 |
