...
*デスクトップアプリ：左側に操作項目、右側にミリ単位のライブプレビュー
（ここではウェブサイト `www.deadlinedriven.dev` を表示。QR コードの初期値と同じ内容です）。*

**「画像」タブ** – 写真・ロゴ・ベクター画像をしきい値・ディザリング・反転・回転・
引き伸ばし・余白の切り取りで調整できます。プレビューは印刷前に白黒の結果を表示します：

![X3 サーマルプリンター – 画像タブ（しきい値・ディザリング・反転・回転・引き伸ばし）](docs/images/gui-image-ja.png)

*「画像」タブと同梱のサンプル画像（`assets/sample.jpg`）：しきい値 190 %、ディザリング有効、
mm 単位の寸法、青いハンドルでドラッグ／拡大縮小。設定は保存され次の印刷にも使われます。*
