...
![X3 サーマルプリンター – バーコード / QR タブ：ウェブサイト www.deadlinedriven.dev の QR コード](docs/images/gui-qr-ja.png)

***「バーコード / QR」タブ**：「QR コード」を選び、内容の種類（ウェブサイト、Wi-Fi、
連絡先、メール、電話、SMS、位置情報、フリーテキスト）を指定します。フォームの下の欄に
実際にコードへ入る内容が表示されます – ここでは `https://www.deadlinedriven.dev`。*
...
* **お使いのシステムの言語で起動**（日本語・English・ドイツ語・中国語）。インストーラーの
  メッセージとメニュー項目も同じ言語になります。変更は「設定 → デザイン → 言語」
...
| **接続できなくなった**（`timed out`） | Bluetooth のセッションが古いまま残っています（X3 は同時に 1 台のみ接続可）。ドライバーが自動で切断して再試行します（最初の印刷は数秒長くなります）。手動なら `bluetoothctl disconnect <MAC>`、またはプリンターの電源を入れ直してください |
| `Device or resource busy` | 前のジョブをまだ印刷中です。少し待つとドライバーが自動で再試行します |
